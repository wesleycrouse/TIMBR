    ...
    for (i in 1:iterations){
      ...
      if (update.M){
        ...
        for (j in j.order){
          ...
          MC.space <- lapply(1:(K+1), function(x){M.list[j] <- x; C[M.list,]})
...
#' @keywords internal
TIMBR.old <- function(y, prior.D, prior.M, prior.phi.b=1, samples=10000, samples.ml=10000, Z=NULL, W=NULL, verbose=T){
