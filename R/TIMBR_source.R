...
#' @keywords internal
TIMBR2 <- function(y, prior.D, prior.M, prior.phi.v=2, samples=10000, Z=NULL, W=NULL, calc.lnBF=T, samples.ml=1000, verbose=T, stop.on.error=F){
    ...
    nglm.hyperparameters.ml <- function(MC, calc.partial.ml=T){
      ...
      if (calc.partial.ml){
        partial.ln.ml <- -0.5*kappa.star*log(psi.star) - 0.5*d*log(phi.sq) + 
          #0.5*determinant(V.star)$modulus[1]
          sum(log(diag(L.Vt.inv)))
        return.list$partial.ln.ml <- partial.ln.ml
      }
